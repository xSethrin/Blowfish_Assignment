...
//May 8, 2010.
//Ported from my C# BLOWFISHB code which was ported from the JavaScript crypto library found here:
//  http://etherhack.co.uk/symmetric/BLOWFISHB/BLOWFISHB.html
//Complies with the test vectors:  http://www.schneier.com/code/vectors.txt
//Description:
//  BLOWFISHB is a keyed, symmetric block cipher, designed in 1993 by Bruce Schneier and
//  included in a large number of cipher suites and encryption products. BLOWFISHB provides
...
//More Information: http://www.schneier.com/paper-BLOWFISHB-fse.html and http://en.wikipedia.org/wiki/BLOWFISHB_cipher
...
-   BLOWFISHB is only as secure as the encryption key you provide. To create a key from a password,
...
#ifndef BLOWFISHB_INCLUDED
#define BLOWFISHB_INCLUDED
...
class BLOWFISHB{
    ...
    #define ROUNDS 16
    public:
        BLOWFISHB(std::string hexKey);
        BLOWFISHB(byte* cipherKey, int keylength);
    ...
    protected:
        ...
        byte* Crypt_ECB(byte* data, int length, int* newlength, void (BLOWFISHB::*CryptBlock)(byte*, int offset), bool decrypt);
        byte* Crypt_CBC(byte* data, int length, int* newlength, void (BLOWFISHB::*CryptBlock)(byte*, int offset), bool decrypt);
    ...
    BLOWFISHB::BLOWFISHB(std::string hexKey)
    {
        IvSet = false;
    ...
    int BLOWFISHB::hex2dec(char hex)
    {
        if('a' <= hex && hex <= 'f')
            return 10 + (hex - 'a');
        if('A' <= hex && hex <= 'F')
    ...
    BLOWFISHB::BLOWFISHB(byte* cipherKey, int keyLength)
    {
        IvSet = false;
        SetupKey(cipherKey, keyLength);
    }

    byte* BLOWFISHB::Encrypt_ECB(byte* data, int length, int* newlength)
    {
        return Crypt_ECB(data,length, newlength, &BLOWFISHB::Encrypt_Block, false);
    }

    byte* BLOWFISHB::Decrypt_ECB(byte* data, int length, int* newlength)
    {
        return Crypt_ECB(data,length, newlength, &BLOWFISHB::Decrypt_Block, true);
    }

    byte* BLOWFISHB::Encrypt_CBC(byte* data, int length, int* newlength)
    {
        return Crypt_CBC(data,length, newlength, &BLOWFISHB::Encrypt_Block, false);
    }

    byte* BLOWFISHB::Decrypt_CBC(byte* data, int length, int* newlength)
    {
        return Crypt_CBC(data,length, newlength, &BLOWFISHB::Decrypt_Block, true);
    }

    std::string BLOWFISHB::Encrypt_CBC(std::string data)
    {
    ...
    std::string BLOWFISHB::Decrypt_CBC(std::string data)
    {
        if(data.length() % 2 != 0)
            throw 2;
    ...
    std::string BLOWFISHB::byteToHex(unsigned char x)
    {
    ...
    byte* BLOWFISHB::padData(byte* data, int length, int* paddedLength, bool decrypt, bool IvSpace = false)
    {
        int offset = 0;
        int dataoffset = 0;
        if(decrypt)
    ...
    int BLOWFISHB::findPaddingEnd(byte* data, int length)
    {
        int i = length;
    ...
    byte* BLOWFISHB::Crypt_ECB(byte* data, int length, int* newlength, void (BLOWFISHB::*CryptBlock)(byte*, int ), bool decrypt)
    {
        byte* pData;
    ...
    byte* BLOWFISHB::Crypt_CBC(byte* data, int length, int* newlength, void (BLOWFISHB::*CryptBlock)(byte*, int ), bool decrypt)
    {
        byte* pData;
        if(!decrypt && !IvSet)
            SetRandomIV();
    ...
    void BLOWFISHB::SetIV(byte* newIV)
    {
        IvSet = true;
        for(int i = 0; i < 8; i++)
            IV[i] = newIV[i];
    ...
    void BLOWFISHB::Encrypt_Block(byte* block, int offset)
    {
    ...
    void BLOWFISHB::Decrypt_Block(byte* block, int offset)
    {
        setblock(block,offset);
        decipher();
        getblock(block,offset);
    ...
    void BLOWFISHB::setblock(byte* block, int offset)
    {
    ...
    void BLOWFISHB::getblock(byte* block, int offset)
    {
        //TODO: CHECK ENDIANNESS
    ...
    void BLOWFISHB::SetupKey(byte* cipherKey, int length)
    {
        if(length > 56)
        {
            throw 56;
    ...
    unsigned int BLOWFISHB::round(unsigned int a, unsigned int b, unsigned int n)
    {
        //TODO: CHECK ENDIANNESS
