...
//May 8, 2010.
//Ported from my C# BLOWFISHA code which was ported from the JavaScript crypto library found here:
//  http://etherhack.co.uk/symmetric/BLOWFISHA/BLOWFISHA.html
//Complies with the test vectors:  http://www.schneier.com/code/vectors.txt
//Description:
//  BLOWFISHA is a keyed, symmetric block cipher, designed in 1993 by Bruce Schneier and
//  included in a large number of cipher suites and encryption products. BLOWFISHA provides
...
//More Information: http://www.schneier.com/paper-BLOWFISHA-fse.html and http://en.wikipedia.org/wiki/BLOWFISHA_cipher
...
-   BLOWFISHA is only as secure as the encryption key you provide. To create a key from a password,
...
#ifndef BLOWFISHA_INCLUDED
#define BLOWFISHA_INCLUDED
...
class BLOWFISHA{
    ...
    #define ROUNDS 16
    public:
        BLOWFISHA(std::string hexKey);
        BLOWFISHA(byte* cipherKey, int keylength);
    ...
    protected:
        ...
        byte* Crypt_ECB(byte* data, int length, int* newlength, void (BLOWFISHA::*CryptBlock)(byte*, int offset), bool decrypt);
        byte* Crypt_CBC(byte* data, int length, int* newlength, void (BLOWFISHA::*CryptBlock)(byte*, int offset), bool decrypt);
    ...
    BLOWFISHA::BLOWFISHA(std::string hexKey)
    {
        IvSet = false;
    ...
    int BLOWFISHA::hex2dec(char hex)
    {
        if('a' <= hex && hex <= 'f')
            return 10 + (hex - 'a');
        if('A' <= hex && hex <= 'F')
    ...
    BLOWFISHA::BLOWFISHA(byte* cipherKey, int keyLength)
    {
        IvSet = false;
        SetupKey(cipherKey, keyLength);
    }

    byte* BLOWFISHA::Encrypt_ECB(byte* data, int length, int* newlength)
    {
        return Crypt_ECB(data,length, newlength, &BLOWFISHA::Encrypt_Block, false);
    }

    byte* BLOWFISHA::Decrypt_ECB(byte* data, int length, int* newlength)
    {
        return Crypt_ECB(data,length, newlength, &BLOWFISHA::Decrypt_Block, true);
    }

    byte* BLOWFISHA::Encrypt_CBC(byte* data, int length, int* newlength)
    {
        return Crypt_CBC(data,length, newlength, &BLOWFISHA::Encrypt_Block, false);
    }

    byte* BLOWFISHA::Decrypt_CBC(byte* data, int length, int* newlength)
    {
        return Crypt_CBC(data,length, newlength, &BLOWFISHA::Decrypt_Block, true);
    }

    std::string BLOWFISHA::Encrypt_CBC(std::string data)
    {
    ...
    std::string BLOWFISHA::Decrypt_CBC(std::string data)
    {
        if(data.length() % 2 != 0)
            throw 2;
    ...
    std::string BLOWFISHA::byteToHex(unsigned char x)
    {
    ...
    byte* BLOWFISHA::padData(byte* data, int length, int* paddedLength, bool decrypt, bool IvSpace = false)
    {
        int offset = 0;
        int dataoffset = 0;
        if(decrypt)
    ...
    int BLOWFISHA::findPaddingEnd(byte* data, int length)
    {
        int i = length;
    ...
    byte* BLOWFISHA::Crypt_ECB(byte* data, int length, int* newlength, void (BLOWFISHA::*CryptBlock)(byte*, int ), bool decrypt)
    {
        byte* pData;
    ...
    byte* BLOWFISHA::Crypt_CBC(byte* data, int length, int* newlength, void (BLOWFISHA::*CryptBlock)(byte*, int ), bool decrypt)
    {
        byte* pData;
        if(!decrypt && !IvSet)
            SetRandomIV();
    ...
    void BLOWFISHA::SetIV(byte* newIV)
    {
        IvSet = true;
        for(int i = 0; i < 8; i++)
            IV[i] = newIV[i];
    ...
    void BLOWFISHA::Encrypt_Block(byte* block, int offset)
    {
    ...
    void BLOWFISHA::Decrypt_Block(byte* block, int offset)
    {
        setblock(block,offset);
        decipher();
        getblock(block,offset);
    ...
    void BLOWFISHA::setblock(byte* block, int offset)
    {
    ...
    void BLOWFISHA::getblock(byte* block, int offset)
    {
        //TODO: CHECK ENDIANNESS
    ...
    void BLOWFISHA::SetupKey(byte* cipherKey, int length)
    {
        if(length > 56)
        {
            throw 56;
    ...
    unsigned int BLOWFISHA::round(unsigned int a, unsigned int b, unsigned int n)
    {
        //TODO: CHECK ENDIANNESS
